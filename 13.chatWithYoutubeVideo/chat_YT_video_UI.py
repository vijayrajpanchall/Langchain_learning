...
# Page configuration
st.set_page_config(page_title="YouTube Transcript RAG", page_icon="🎥", layout="wide")
...
with col2:
    ...
    if st.session_state.video_processed and st.session_state.vector_store:
        st.success(
            f"✅ Ready to answer questions about video: {st.session_state.current_video_id}"
        )
        ...
        if ask_button and question:
            try:
                with st.spinner("Thinking..."):
                    ...
                    # Setup LLM
                    llm = ChatOpenAI(
                        model="gpt-3.5-turbo", temperature=temperature, max_tokens=500
                    )
